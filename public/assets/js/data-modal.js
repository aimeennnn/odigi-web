// Data Modal JavaScript (Drag & Drop Upload)
(function () {
    "use strict";
    ...
    function initDragDropUpload() {
        const dz = document.getElementById("dataDropZoneInline");
        const choose = document.getElementById("dataChooseFiles");
        const input = document.getElementById("fileInputMulti");
        const list = document.getElementById("dataFileList");
        const tempHolder = document.getElementById("tempPathsHolder");
        ...
        function addProgressRow(name, tempPath = null) {
            ...
            row.className =
                "p-2 border rounded bg-white d-flex align-items-center justify-content-between gap-2";
            row.innerHTML = `
        ...
        function startTempUpload(files) {
            ...
            const uploadUrl = "/data/upload-temp-new";
            ...
            xhr.onload = () => {
                try {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300 && res.success) {
                        ...
                        const submitBtn =
                            document.getElementById("btnSimpanData");
                        if (submitBtn) {
        ...
        updateFileUploadIcon = function () {
            const iconSpan = document.querySelector("#fileUploadIcon i");
            if (iconSpan) {
                console.log(
                    "Updating file upload icon, pending files:",
                    pending.length
                ); // Debug log
                if (pending.length > 0) {
        ...
        window.updateFileUploadIcon = updateFileUploadIcon;
    }
    ...
    function initFormValidation() {
        ...
        if (form) {
            form.addEventListener("submit", function (e) {
                console.log(
                    "Form submit event triggered, pending files:",
                    pending.length
                );
                ...
                console.log("Temp paths inputs found:", tempInputs.length);
                tempInputs.forEach((input, index) => {
                    console.log(`temp_paths[${index}]:`, input.value);
                });

                // Validasi akan dilakukan di click handler tombol
            });
        }

        // Button state management
        const submitBtn = document.getElementById("btnSimpanData");
        if (submitBtn) {
            ...
            if (modal) {
                modal.addEventListener("shown.bs.modal", function () {
                    ...
                    if (form) {
                        ...
                        const fileInput =
                            document.getElementById("fileInputMulti");
                        if (fileInput) {
                    ...
                    setTimeout(() => {
                        updateFileUploadIcon();
                        console.log(
                            "Modal reset completed, pending files:",
                            pending.length
                        );
                    }, 100);

                    console.log(
                        "Modal opened, submit button enabled, files reset"
                    );
                });
                ...
                modal.addEventListener("hidden.bs.modal", function () {
                    pending = [];
                    console.log("Modal closed, pending reset");
                });
            }
            ...
            submitBtn.addEventListener("click", function (e) {
                ...
                const form = document.querySelector("#modalTambahData form");
                const jenisData = form?.querySelector(
                ...
                if (!jenisData || jenisData.value.trim() === "") {
                    e.preventDefault();
                    alert("Mohon pilih jenis data!");
                    return false;
                }
    ...
    function init() {
        ...
        setTimeout(() => {
            if (typeof updateFileUploadIcon === "function") {
                updateFileUploadIcon();
                console.log("Initial file upload icon set");
            }
