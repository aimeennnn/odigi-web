@php
    ...
    $sort_link = function ($col, $label) {
        ...
        $icon =
            '<span class="ms-1" style="font-size:1rem;vertical-align:middle;">
            <span ' .
            $upColor .
            '>&uarr;</span><span ' .
            $downColor .
...
@section('main-content')
                    ...
                    <div class="col-md-9">
                        ...
                        <select class="form-select" id="filter_bank" name="filter_bank">
                            ...
                            @foreach (\App\Models\Bank::namaBankList() as $key => $label)
                                <option value="{{ $key }}" {{ request('filter_bank') == $key ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3 d-flex gap-2">
                        <button type="submit" class="btn btn-bank-blue w-100"><i
                                class="bi bi-search me-1"></i>Filter</button>
                        @if (request('register_id'))
                            <a href="{{ route('bank.index', ['register_id' => request('register_id')]) }}"
                                class="btn btn-bank-danger w-100"><i class="bi bi-x-circle me-1"></i>Reset</a>
                        @else
                            <a href="{{ route('bank.index') }}" class="btn btn-bank-danger w-100"><i
            ...
            <div class="card-body">
                <div class="d-flex justify-content-end mb-3 gap-2">
                    @if (\App\Helpers\RoleHelper::canCreate('bank'))
                        <button class="btn btn-bank" data-bs-toggle="modal" data-bs-target="#modalTambahBank"><i
                                class="bi bi-plus-circle me-1"></i>Tambah Data</button>
                    @endif
                    @if (\App\Helpers\RoleHelper::canDelete('bank'))
                        <button id="btnBulkBank" type="button" class="btn btn-bank-danger"><i
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <select class="form-select d-inline-block w-auto" style="min-width:60px;"
                            onchange="changePerPageBank(this.value)">
                    ...
                    <div class="input-group" style="max-width: 250px;">
                        ...
                        <input type="text" class="form-control" placeholder="CARI ARSIP..." id="searchBank"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($item->file)
                                                ...
                                                <div class="d-flex flex-column gap-1"
                                                    ...
                                                    @foreach ($files as $p)
                                                        @php
                                                            ...
                                                            $encryptedUrl = \App\Helpers\UrlEncryptionHelper::encryptFileUrl(
                                                                $item->id_bank,
                                                                $loop->index,
                                                                'bank',
                                                                $item->nama_bank,
                                                        ...
                                                        <div class="file-item d-flex align-items-center gap-2">
                                                            <a href="{{ $url }}" data-url="{{ $url }}"
                                                                data-type="{{ $isImage ? 'image' : (strtolower(pathinfo($p, PATHINFO_EXTENSION)) === 'pdf' ? 'pdf' : 'other') }}"
                                                                class="bank-file-link text-decoration-none"
                                                                title="Lihat file" style="flex:0;">
                                                                @if ($isImage)
                                                                    <img src="{{ $url }}" alt="Dokumen"
                                                                        style="width:36px;height:36px;object-fit:cover;border-radius:6px;border:1px solid #e9ecef;">
                                                                @else
                                            ...
                                            @else
                                                <div class="text-center text-muted"><small>File corrupt/hilang</small>
                                                </div>
                                            @endif
                                        @else
                                            <div class="text-center">
                                                <span class="text-muted"><i class="bi bi-file-earmark-x me-1"></i>Belum
                                                    ada file</span>
                                            </div>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        @if ($item->hasil)
                                            ...
                                            @php
                                                ...
                                                $encryptedUrlH = \App\Helpers\UrlEncryptionHelper::encryptFileUrl(
                                                    $item->id_bank,
                                                    0,
                                                    'bank_hasil',
                                                    $item->nama_bank,
                                            ...
                                            <div class="d-flex flex-column align-items-center">
                                                <div class="d-inline-flex align-items-center gap-2 px-3 py-2"
                                                    style="background:#f8fbff;border:1px solid #e9ecef;border-radius:12px;">
                                                    <a href="{{ $urlH }}" data-url="{{ $urlH }}"
                                                        data-type="{{ $isImgH ? 'image' : 'other' }}"
                                                        class="bank-file-link text-decoration-none" title="Lihat file">
                                                        @if ($isImgH)
                                                            <img src="{{ $urlH }}" alt="Hasil"
                                                                style="width:34px;height:34px;object-fit:cover;border-radius:8px;border:1px solid #e9ecef;">
                                                        @else
                                                            <i class="bi bi-file-earmark" style="font-size:1.15rem;"></i>
                                                        @endif
                                                    </a>
                                                    <span class="badge bg-success"
                                                        style="font-size:0.8rem;border-radius:8px;padding:6px 12px;">
                                                        <i class="bi bi-check-circle me-1"></i>Valid
                                                    </span>
                                                </div>
                                                <small class="text-muted d-block mt-2 text-center">
                                                    <i
                                                        class="bi bi-clock me-1"></i>{{ \Carbon\Carbon::parse($item->updated_at)->format('d/m/Y H:i') }}
                                                </small>
                                            </div>
                                        @else
                                            <div class="d-inline-flex align-items-center gap-2 px-3 py-2"
                                                ...
                                                {{-- LOGIKA GANTI NAMA LABEL --}}
                                                @php
                                                    $labelTampil = $item->status ?? 'Dalam Proses';
                                                    // Kalau status aslinya ada kata 'n8n' atau 'process', ganti tampilannya jadi 'Proses Audit'
                                                    if (
                                                        stripos($labelTampil, 'n8n') !== false ||
                                                        stripos($labelTampil, 'process') !== false
                                                    ) {
                                                        $labelTampil = 'Proses Audit';
                                                    }
                                                @endphp

                                                <span class="badge status-label"
                                                    data-id="{{ $item->id_bank ?? $item->id }}"
                                                    style="background:#e3f0ff; color:#4F8CFF; font-size:0.8rem;border-radius:8px;padding:6px 12px;">
                                                    <i class="bi bi-clock me-1"></i>{{ $labelTampil }}
                                                </span>
                                            </div>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2 align-items-center justify-content-center">
                                            @php
                                                $encId = strtr(
                                                    \Illuminate\Support\Facades\Crypt::encryptString($item->id_bank),
                                                    ['+' => '-', '/' => '_', '=' => '.'],
                                                );
                                                $regParam = request('register_id')
                                            ...
                                            @if (\App\Helpers\RoleHelper::canViewDetailBank())
                                                <a href="{{ route('bank.show', $encId) . $regParam }}"
                                                    class="action-icon view-icon" title="Lihat Detail">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                            @endif
                                            ...
                                            {{-- Tombol Hapus --}}
                                            @if (\App\Helpers\RoleHelper::canDelete('bank'))
                                                <form action="{{ route('bank.destroy', $item->id_bank) }}" method="POST"
                                                    class="d-inline"
        ...
        @if ($bankData->hasPages())
            <div class="d-flex justify-content-between align-items-center mt-4 px-3">
                <div class="pagination-info">
                    <span class="text-muted" style="font-size: 14px;">Showing {{ $bankData->firstItem() ?? 0 }} to
                        {{ $bankData->lastItem() ?? 0 }} of {{ $bankData->total() }} entries</span>
                </div>
    ...
    @if ($regEnc)
        ...
        @if ($register)
            <div class="modal fade" id="modalDokumenRegister" tabindex="-1" aria-labelledby="modalDokumenRegisterLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-xl">
                    <div class="modal-content" style="border-radius: 18px;">
                        <div class="modal-header"
                            style="background: linear-gradient(90deg, #1dd1a1 0%, #49bbca 100%); color: white; border-radius: 18px 18px 0 0;">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="info-item">
                                                        ...
                                                        <div class="info-value">
                                                            {{ \Carbon\Carbon::parse($register->tgl_pengajuan)->format('d/m/Y') }}
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="info-item">
                                                        ...
                                                        <div class="info-value">
                                                            <span
                                                                class="badge {{ $register->jenis_entitas == 'perorangan' ? 'bg-primary' : 'bg-success' }}">
                                                ...
                                                <div class="row g-3">
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="info-item">
                                                            ...
                                                            <div class="info-value">
                                                                {{ \App\Models\Register::jenisKelaminList()[$register->jns_kelamin] ?? $register->jns_kelamin }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="info-item">
                                                            ...
                                                            <div class="info-value">
                                                                {{ \App\Models\Register::jenisIdentitasList()[$register->jns_identitas] ?? $register->jns_identitas }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="info-item">
                                                            ...
                                                            <div class="info-value">
                                                                {{ \App\Models\Register::pekerjaanList()[$register->pekerjaan] ?? $register->pekerjaan }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                ...
                                                <div class="row g-3">
                                                    <div class="col-md-6">
                                                        <div class="info-item">
                                                            ...
                                                            <div class="info-value">{{ $register->nama_badan_usaha }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="info-item">
                                                            ...
                                                            <div class="info-value">{{ $register->jenis_dokumen_usaha }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <div class="info-item">
                                                            ...
                                                            <div class="info-value">{{ $register->nomor_legalitas_usaha }}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                            ...
                                            <div class="row g-3">
                                                <div class="col-md-6">
                                                    <div class="info-item">
                                                        ...
                                                        <div class="info-value">{{ $register->jenis_pengajuan_label }}
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="info-item">
                                                        ...
                                                        <div class="info-value">Rp
                                                            {{ number_format($register->nominal_pengajuan, 0, ',', '.') }}
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                ...
                                                <div class="col-12">
                                                    <div class="info-item">
                                                        ...
                                                        <div class="info-value">
                                                            {{ \App\Models\Register::jaminanList()[$register->jaminan] ?? $register->jaminan }}
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <img id="bankPreviewImage" src="" alt="Preview"
...
@push('scripts')
    {{-- 1. LOAD JQUERY DULU (WAJIB ADA BIAR TIDAK ERROR '$ is not defined') --}}
    ...
    {{-- 3. SCRIPT AUTO RELOAD (VERSI FINAL) --}}
    <script>
        $(document).ready(function() {
            console.log("🚀 SCRIPT AUTO-RELOAD SIAP! (jQuery Loaded)");

            let monitoringIds = [];

            // Cari data yang statusnya "Dalam Proses"
            ...
            // Jika ada yang perlu dipantau
            if (monitoringIds.length > 0) {
                console.log("🕵️‍♂️ Memantau ID: ", monitoringIds);

                setInterval(function() {
                    monitoringIds.forEach(function(id) {
                        $.ajax({
                            url: '/bank/check-status/' + id,
                            method: 'GET',
                            cache: false,
                            success: function(response) {
                                // Ambil status & hasil dengan aman
                                let status = response.status ? response.status
                                    .toLowerCase() : '';
                                let hasil = response.hasil;

                                // Syarat Reload: Status 'selesai'/'valid' ATAU file hasil sudah ada
                                if (status.includes('selesai') || status.includes(
                                        'valid') || hasil != null) {
                                    console.log("✅ JOB DONE! Reloading page...");
                                    location.reload();
                                }
                            },
                            error: function(err) {
                                console.warn("⚠️ Gagal cek ID " + id);
                            }
                        });
                    });
                }, 4000); // Cek tiap 4 detik
            } else {
                console.log("😴 Tidak ada data process yang perlu dipantau.");
            }
        });
    </script>
    ...
    {{-- 5. Script Fix Modal Bootstrap --}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Cek apakah bootstrap sudah didefinisikan
